<?php
/**
 * spark Production Report - Independent Shift Summaries
 */
...
$s1_Out_Data = getShiftStats($conn, $viewDate, $safe2Id, $sparkId, false, $cutoffMinutes);
$s1_In_Data  = getShiftStats($conn, $viewDate, $sparkId, $safe2Id, false, $cutoffMinutes);
$s1_Wip      = $s1_In_Data['weight'] - $s1_Out_Data['weight'];

// 4. CALCULATE SHIFT 2 ONLY
$s2_Out_Data = getShiftStats($conn, $viewDate, $safe2Id, $sparkId, true, $cutoffMinutes);
$s2_In_Data  = getShiftStats($conn, $viewDate, $sparkId, $safe2Id, true, $cutoffMinutes);
$s2_Wip      = $s2_In_Data['weight'] - $s2_Out_Data['weight'];
...
$wip      = $totalIn - $totalOut;
$hasVariance = (abs($wip) > 0.0001); 

// 6. LISTING SQL
$sqlList = "SELECT TL.Id as TransactionLogId, I.CreatedOn as InvDate, TL.SourceDepartmentId, TL.Weight, I.ProductName, 
...
<head>
    <meta charset="UTF-8">
    <title>spark Production Report</title>
...
<div class="container pb-5">
    ...
    <div class="mb-4">
        ...
        <a href="audit_trail.php" class="btn btn-outline-info btn-sm rounded-pill px-3 ms-2"><i class="bi bi-shield-check"></i> Audit Trail</a>
    </div>
        ...
        <table class="table table-dark table-hover mb-0 align-middle">
            <thead>
                <tr class="text-secondary small uppercase">
                    ...
                    <th class="text-end pe-4">Admin</th>
                </tr>
            </thead>
            <tbody>
                <?php 
                $stmtList = executeQuery($conn, $sqlList, [$viewDate, $safe2Id, $sparkId, $sparkId, $safe2Id]);
                ...
                ?>
                    <tr class="shift-divider text-warning">
                        <td colspan="5" class="text-center py-2 small fw-bold">--- SHIFT 2 LOGS STARTING <?= sprintf('%02d:%02d', $shiftHour, $shiftMinute) ?> ---</td>
                    </tr>
                <?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td class="text-end fw-bold <?= $isOut ? 'text-out' : 'text-in' ?>">
                            <?= number_format($row['Weight'], 3) ?>g
                        </td>
                        <td class="text-end pe-4">
                            <a class="btn btn-outline-warning btn-sm" href="admin_edit.php?id=<?= (int)$row['TransactionLogId'] ?>&return=<?= urlencode(basename(__FILE__)) ?>&date=<?= urlencode($viewDate) ?>&shift_h=<?= urlencode($shiftHour) ?>&shift_m=<?= urlencode($shiftMinute) ?>">Edit</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
